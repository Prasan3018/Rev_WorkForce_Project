...
public class EmployeeMenu {
	...
	public void showMenu(Employee emp) {
		...
		while (true) {
			...
			case 4:
				int[] balance = employeeService
						.viewLeaveBalance(emp.getEmpId());
			...
			case 5:
				...
				boolean applied = employeeService.applyLeave(emp.getEmpId(),
			...
			case 6: {
				...
				try {
					java.sql.ResultSet rs = employeeService.viewMyLeaves(emp
							.getEmpId());

					System.out.println("\n----- My Leave Applications -----");
					System.out
							.println("LEAVE_ID | TYPE | FROM | TO | STATUS | REASON");
			...
			case 7: {
				...
				try {
					java.sql.ResultSet rs = employeeService.viewHolidays();
			...
			case 8: {
				...
				try {
					...
					boolean cancelled = employeeService.cancelLeave(leaveId,
							emp.getEmpId());
					...
					if (cancelled) {
						...
						System.out
								.println("Unable to cancel leave (maybe already approved).");
					}
			...
			case 9: {
				...
				try {
					java.sql.ResultSet rs = employeeService
							.viewNotifications(emp.getEmpId());
			...
			case 10: {
				...
				boolean submitted = employeeService.submitPerformanceReview(
						emp.getEmpId(), deliverables, accomplishments,
						improvements, rating);

				if (submitted) {
					System.out
							.println("Performance review submitted successfully.");
				} else {
			...
			case 11: {
				...
				try {
					java.sql.ResultSet rs = employeeService
							.viewMyPerformanceReview(emp.getEmpId());
			...
			case 12: {
				...
				boolean added = employeeService.addGoal(emp.getEmpId(), desc,
						deadline, priority, metrics);
			...
			case 13: {
				...
				try {
					java.sql.ResultSet rs = employeeService.viewGoals(emp
							.getEmpId());

					System.out.println("\n----- My Goals -----");
					System.out
							.println("ID | DESCRIPTION | DEADLINE | PRIORITY | PROGRESS | METRICS");
			...
			case 14: {
				...
				boolean updated = employeeService.updateGoalProgress(goalId,
						progress);
